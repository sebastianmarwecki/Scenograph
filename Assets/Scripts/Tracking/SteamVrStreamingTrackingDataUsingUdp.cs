...
namespace Assets.Scripts
{
    ...
    public class SteamVrStreamingTrackingDataUsingUdp : MonoBehaviour {
        ...
        public void Order()
        {
            ...
            //get actives and order!
            var posZ = new List<float>();
            var ids = new List<int>();
            for (var i = 0; i < _steamTrackedObjectsInitial.Count; i++)
            {
                var sto = _steamTrackedObjectsInitial.First(s => (int)s.index == i);
                var pos = transform.InverseTransformPoint(sto.gameObject.GetComponent<SteamVR_Offset>().Child.position);
                var dist = sto.gameObject.activeSelf ? pos.z : 1000f + i;
                posZ.Add(dist);
                ids.Add((int)sto.index);
                //Debug.Log((int)sto.index + " " + dist);
            }

            var poszOrdered = new List<float>(posZ);
            poszOrdered.Sort();
            //var orderedIds = ids.OrderBy(id => posZ[ids.IndexOf(id)]).ToList();
            var orderedIds = ids.Select(id => ids[poszOrdered.IndexOf(posZ[ids.IndexOf(id)])]).ToList();
            //for (var i = 0; i < orderedIds.Count; i++)
            //{
            //    var order = orderedIds[i];      
            //    Debug.Log(order);
            //}
            var rest = new List<SteamVR_TrackedObject>(_steamTrackedObjectsInitial);
            rest = rest.Where(r => !_objectsStart.Contains(r)).ToList();

            for (var i = 0; i < _objectsStart.Count; i++)
            {
                //var index = _steamTrackedObjects.IndexOf(_objectsStart[i]);
                
                //_steamTrackedObjects[index].index 
                _objectsStart[i].index = (SteamVR_TrackedObject.EIndex)orderedIds.IndexOf(i);
                Debug.Log(_objectsStart[i].gameObject.name + " set to " + orderedIds.IndexOf(i));
            }
            for (var i = 0; i < rest.Count; i++)
            {
                //var index = _steamTrackedObjects.IndexOf(rest[i]);
                //_steamTrackedObjects[index].index 
                rest[i].index = (SteamVR_TrackedObject.EIndex)orderedIds.IndexOf(_objectsStart.Count + i);//[_objectsStart.Count + i];
            }
        }
